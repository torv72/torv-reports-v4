...
if(sample_dates_per_type$n == 1) {
  ...
  all_om_data_comms <- 
    ...
    mutate(type_date = paste0(case_when(reference_only == "for reference" ~ paste0("\n- Just for reference, since no **",
                                        ...
                                        TRUE ~ "\n- T"),
                              "he **", sample_description_number_2, "** samples reported on ", verbaliseR::prettify_date(date_sample_submitted, "US"),
                              " were as follows:"),
  ...
  all_om_data_comms <- 
    all_om_longitudinal_table %>%
    group_by(sample_description_number_2, depth) %>%
    arrange(date_sample_submitted) %>%  # ← ADD THIS LINE TO ENSURE CHRONOLOGICAL ORDER
    ...
    mutate(type_date = paste0(case_when(reference_only == "for reference" ~ paste0("\n- Just for reference, since no **",
                                        ...
                                        is.na(date_sample_submitted_date_2) ~ paste0("\n- The **", sample_description_number_2_date_1,
                                                                                     "** samples reported on ", verbaliseR::prettify_date(date_sample_submitted_date_1, "US"),
                                                                                     " were as follows:"),
                                        TRUE ~ "\n- C"),
                              case_when(!is.na(date_sample_submitted_date_2) ~ paste0("omparing the **", sample_description_number_2_date_2,  # ← CHANGE FROM date_1 TO date_2
                                                                                      "** samples reported on ", verbaliseR::prettify_date(date_sample_submitted_date_2, "US"),  # ← CORRECT (newer date)
                                                                                      " to their most recent previous samples (",
                                                                                      verbaliseR::prettify_date(date_sample_submitted_date_1, "US"), ")."),  # ← CORRECT (older date)
                                        TRUE ~ "")),
           difference = paste0("\n  * At a depth of **", depth_date_1, "**, the OM content ",
                               case_when(
                                 ...
                                 janitor::round_half_up(avg_measurement_result_date_2, 2) - janitor::round_half_up(avg_measurement_result_date_1, 2)  > 0 ~  # ← SWAP: date_2 - date_1
                                   paste0("increased from ",  # ← SWAP: now says "increased" when date_2 > date_1
                                          janitor::round_half_up(avg_measurement_result_date_1, 2),
                                          "% to ",
                                          janitor::round_half_up(avg_measurement_result_date_2, 2), "%."),
                                 janitor::round_half_up(avg_measurement_result_date_2, 2) - janitor::round_half_up(avg_measurement_result_date_1, 2) < 0 ~  # ← SWAP
                                   paste0("decreased from ",  # ← SWAP
                                          janitor::round_half_up(avg_measurement_result_date_1, 2),
                                          "% to ",
                                          janitor::round_half_up(avg_measurement_result_date_2, 2), "%."),
           ...
           commentary = case_when(is.na(date_sample_submitted_date_2) ~ paste0(difference, "."),
                                  TRUE ~ paste0(difference, ", a change of **", janitor::round_half_up(avg_measurement_result_date_2 - avg_measurement_result_date_1, 2), "%**.")))  # ← CORRECT: date_2 - date_1
