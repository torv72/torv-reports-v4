...
header-includes:
  ...
  - \definecolor{torvmoss}{HTML}{6D7F56}
  - \newtcolorbox{insight}{breakable,colback=white,colframe=torvmoss,leftrule=3pt,rightrule=0pt,toprule=0pt,bottomrule=0pt,arc=3pt,boxsep=4pt,left=6pt,right=6pt,top=4pt,bottom=4pt}
  - \newtcolorbox{context}{breakable,colback=white,colframe=gray!50,leftrule=3pt,rightrule=0pt,toprule=0pt,bottomrule=0pt,arc=3pt,boxsep=4pt,left=6pt,right=6pt,top=4pt,bottom=4pt}
  ...
  - \pagestyle{fancy}
  - \fancyhf{}
  - \fancyhead[L]{\textcolor{gray}{Agronomic Report}}
  - \renewcommand{\headrulewidth}{0pt}
  - \fancyfoot[L]{\footnotesize\textcolor{gray}{TORV, LLC • 260 Woodruff Road, Glenwood Springs, CO 81601 • +1.970.409.9874 • eric@torv.me}}
...
zip_code_list <<- tibble::tribble(
  ...
  "Sun Valley Resort White Clouds Golf Course", 83353,
  "Sun Valley Resort Trail Creek Golf Course", 83353,
  "The Briarwood", 59101,
...
```{r logo-header, out.width="35%", fig.align='left', include=!knitr::is_latex_output()}
...
cat(
  ...
  "\\noindent\\includegraphics[width=2.2in]{assets/torv-logo-wide.png}",
  "\\vspace*{2.8in}",
  "\\par\\noindent{\\Large\\textbf{Agronomic Report}}\\par\\vspace{0.25em}",
  paste0("\\noindent{\\large ", input_params$site_name, "}\\par\\vspace{0.25em}"),
  paste0("\\noindent{\\normalsize ", format(Sys.time(), "%B %d, %Y"), "}\\par\\vspace{0.25em}"),
  "\\noindent{\\normalsize Eric Foerster MG, CGCS}",
  sep = "\n"
)
```
...
cat(
  "\\vfill",
  "\\noindent\\includegraphics[totalheight=0.7cm]{assets/logo-brookside.png}\\newline",
...
\AddToShipoutPictureFG{%
  \AtPageCenter{%
    \makebox[0pt]{%
      \transparent{0.04}\includegraphics[width=4in]{torv-logo.png}%
    }%
  }%
}
```
...
cat("<br><h1 style='color:black;font-size:18pt !important;'><b style='font-weight:600;'>Interactive Agronomic Report</b><br>", input_params$site_name, "</h1>",
  "**Sampling Date:** ",
...
\newpage
\vspace*{-0.8cm}
```{r exec-summary-header, out.width=if(knitr::is_latex_output()) "5.8in" else "900px", out.height=if(knitr::is_latex_output()) "0.44in" else NULL}
...
if (!file.exists(txt_path) | overwrite_report) {
  ...
  if(length(soil_types) > 0) {
    for(soil_type in soil_types) {
      cat("\n\n\\vspace{6mm}\n\\noindent{\\large\\textbf{Nutrient Analysis \U2014 ", str_to_title(soil_type), "}}\n", sep = "")
      
      cat("
All calculations are based on a monthly maximum of", grass_max_n_per_month_per_1000sqft[[soil_type]], "lbs of N/M.\n\n\\medskip", sep = " ")
      # Try to get the comparison text with dates if available
if(exists("chart_info") && soil_type %in% names(chart_info) && !is.null(chart_info[[soil_type]]$comparison_text)) {
  cat("\n", chart_info[[soil_type]]$comparison_text, "\n\n\\medskip", sep = "")
} else {
  cat("
This table shows how sample values have changed between the two most recent comparable testing periods.\n\n\\medskip", sep = "")
}
...
**Topdressing Recommendations:**\n", sep = "")
  ...
  if(input_params$include_sand_fraction) {
    cat("\n\n\\vspace{6mm}\n\\noindent{\\large\\textbf{Sand Fraction}}\n")
...
```{r turf-growth-header, eval=(length(soil_types) > 0), out.width=if(knitr::is_latex_output()) "5.8in" else "900px", out.height=if(knitr::is_latex_output()) "0.44in" else NULL}
...
if(length(water_types > 0)) { # So the section only shows up if there are Water results to report
  ...
  water_sample_descriptions <- water_mod_filtered_database %>%
    ...
    unique() %>%
    .[!tolower(.) %in% tolower(input_params$suppress_water_sources)]
...
<!-- HOW TO READ PAGE — suppressed. To restore: remove these HTML comments, change include=FALSE back to include=!html on the three chunks below. -->
<!-- \newpage -->
<!-- \hypertarget{how-to-read}{} -->
<!-- \vspace*{-0.8cm} -->
```{r r how-to-header, include=FALSE, out.width=if(knitr::is_latex_output()) "5.8in" else "900px", out.height=if(knitr::is_latex_output()) "0.44in" else NULL}
...
<!-- \par -->
<!-- \begin{multicols}{2} -->
```{r how-to-text, results='asis', include=FALSE}
cat("Each plot shows individual sample values as small gray dots and the period average as a larger colored point. The purple line is a linear trend fitted across all sampling dates — it indicates direction, not precision. When averages scatter widely around the trend line, treat the direction as a signal rather than a conclusion.
    \\newline For reference, the blue dotted line marks the site historic average. Organic Matter plots show the TORV average as a gray dotted line instead.")
```
<!-- \columnbreak -->
```{r how-to-graphic, out.width="100%", include=FALSE}
