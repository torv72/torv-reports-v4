...
```{r, results='asis'}
cat(glue("## {{sample_description_number_2}}"))
```
...
torv_color <- torv_green

om_avgs <- 
  om_data %>%
  group_by(measurement_name, depth, plot_date) %>%
  summarize(
    avg_measurement_result = mean(measurement_result, na.rm = TRUE),
...
last_two_dates <- 
  ...
  mutate(
    ...
    text_color = case_when(plot_date == max(plot_date) ~ prismatic::color(torv_color), TRUE ~ prismatic::clr_lighten(torv_color, .15))
  )
...
make_om_plot <- function(plot_measurement_name, plot_depth, draw_beeswarm = beeswarm) {
  ...
  data <- 
    ...
    mutate(
      ...
      tooltip = paste0("<b>", sprintf("%.2f", avg_measurement_result, 2), "%</b><br><span style='font-size:12pt;'>in ", year(plot_date), " (", n, ")</span>")
    )
  ...
  om_plot <- 
    ...
    ggiraph::geom_hline_interactive(
      aes(yintercept = torv_avg, data_id = 1, tooltip = paste0("<span style='font-size:12pt;'>Historic average:</span><br><b>", round(torv_avg, 2), "%</b>")),
    ...
    labs(y = NULL, title = glue::glue("Soil Depth: {plot_depth}")) +
    theme(
...
ggsave(here(root_figure_location, "organic_matter", "trend_plot_om_{{sample_description_number_2}}.png"),
       combined_trends, width = 6.5, height = 4.5, bg = "#ffffff", dpi = 300)
...
om_longitudinal_table_reduced <- 
  om_longitudinal_table %>%
  arrange(desc(date_sample_submitted)) %>%
...
om_longitudinal_table_reduced %>%
  ...
  flextable::colformat_num(suffix = "%") %>%
  flextable::add_header_row(values = c("", gsub(" ", "\n", paste(zoo::as.yearmon(unique(om_longitudinal_table_reduced$date_sample_submitted)))))) %>%
  flextable::border_remove() %>%
  flextable::border(part = "body", border.bottom = officer::fp_border(color = "white", width = 1)) %>%
  flextable::align(part = "all", align = "center") %>%
  flextable::fontsize(size = 10, part = "all") %>%
  flextable::colformat_double(big.mark = ",", digits = 2, na_str = "N/A") %>%
  flextable::bg(part = "body", i = ~ depth == "0-2 cm", j = -1, bg = colorer_02) %>%
  flextable::bg(part = "body", i = ~ depth == "2-4 cm", j = -1, bg = colorer_24) %>%
  flextable::bg(part = "body", i = ~ depth == "4-6 cm", j = -1, bg = colorer_46) %>%
  ...
  flextable::font(fontname = typeface_condensed, part = "all") %>%
  flextable::line_spacing(i = 1, space = 1.3, part = "header") %>%
