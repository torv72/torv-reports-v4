...
om_data <- 
  om_data %>%
  mutate(
    plot_date = as.Date(date_sample_submitted),
    date_tooltip = tooltip_date(date_sample_submitted)
  ) %>%
  group_by(plot_date, date_tooltip, measurement_name, depth)
...
plot_range <- if(year_range[2] - year_range[1] < 5) { 
  range(as_date(ymd(om_data$plot_date)))
} else {
  as_date(date_decimal(year_range))
}

if (year_range[2] - year_range[1] < 5) {
  ...
  om_plot <- 
    ...
    coord_cartesian(clip = "off") +
     scale_x_date(
      ...
      date_labels = ifelse(year_range[2] - year_range[1] < 5, "%b\n%Y", "%Y"),
