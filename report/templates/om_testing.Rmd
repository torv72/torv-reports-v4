...
#if(sample_type == "OM") { type_color <- torv_green }
#if(sample_type == "Soil") { type_color <- torv_orange }

type_color <- torv_green
...
last_two_dates <- om_avgs %>%
  ...
  mutate(label_v_align = case_when(is.na(label_v_align) ~ 1 - lead(label_v_align), TRUE ~ label_v_align),
         text_size = case_when(plot_date == max(plot_date) ~ 3.25, TRUE ~ 2.6),
         text_color = case_when(plot_date == max(plot_date) ~ prismatic::color(type_color), TRUE ~ prismatic::clr_lighten(type_color, .15)))
  ...
  om_plot <- ggplot(data) +
    ...
    { if(draw_beeswarm)
      ...
      ggbeeswarm::geom_beeswarm(
        data = filter(om_data, measurement_name == plot_measurement_name,
                      depth == plot_depth),
    ...
    geom_line(
      aes(x = plot_date, y = avg_measurement_result),
      color = prismatic::clr_lighten(type_color, .3), linewidth = .6, linetype = "solid", alpha = .65 
    ) +
    geom_point(
      aes(x = plot_date, y = avg_measurement_result),
      color = type_color, size = 1.6, 
    ...
    scale_y_continuous(
      labels = function(x) paste0(sprintf("%2.2f", janitor::round_half_up(x, 2)), "%"),
    ...
    theme(
      plot.title = element_text(size = 10, family = typeface_title),
...
# Legend function created in setup.Rmd
om_plots[[4]] <- make_trendline_legend(main_color = type_color)
...
om_display_table <- om_longitudinal_table %>%
  ...
  color(i = 1, color = type_color, part = "header") %>%
...
knitr::include_graphics(glue(here(root_figure_location, "organic_matter"), "/om_plot_{{sample_description_number_2}}.png"))
```
