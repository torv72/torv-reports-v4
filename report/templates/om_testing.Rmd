...
last_two_dates <- om_avgs %>%
  ...
  mutate(label_v_align = case_when(is.na(label_v_align) ~ 1 - lead(label_v_align), TRUE ~ label_v_align),
         ...
         text_color = case_when(plot_date == max(plot_date) ~ prismatic::color(torv_orange), TRUE ~ prismatic::clr_lighten(torv_orange, .15)))
...
make_om_plot <- function(plot_measurement_name, plot_depth, draw_beeswarm = beeswarm) {
  ...
  om_plot <- ggplot(data) +
    ...
    geom_line(
      aes(x = plot_date, y = avg_measurement_result),
      color = prismatic::clr_lighten(torv_orange, .3), linewidth = .6, linetype = "solid", alpha = .65 
    ) +
    geom_point(
      aes(x = plot_date, y = avg_measurement_result),
