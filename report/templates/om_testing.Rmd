...
\newpage
\vspace*{-0.8cm}
```{r, warning=FALSE, message=FALSE, out.width=if(knitr::is_latex_output()) "5.8in" else "900px", out.height=if(knitr::is_latex_output()) "0.58in" else NULL}
...
Organic matter accumulates from roots, organic inputs, and microbial activity. The charts track how OM% at each depth has shifted over time — the trend line indicates direction; dashed lines show the site historic and TORV averages for reference.

The S325 package tests the full sample as submitted, including leaves, stems, and roots. This is why percentages here run higher than a standard soil test, which filters out most of those components.
...
last_two_dates <- 
  ...
  mutate(
    ...
    text_color = case_when(plot_date == max(plot_date) ~ torv_gray, TRUE ~ torv_gray)
  )
...
make_om_plot <- function(plot_measurement_name, plot_depth, draw_beeswarm = input_params$beeswarm) {
  ...
  site_historic <- calculate_om_stats(data$avg_measurement_result)

  om_plot <-
    ggplot(data) +
    ## site historic mean/median as gray dashed line
    geom_hline_interactive(
      aes(yintercept = site_historic, data_id = 2, tooltip = paste0("<span style='font-size:12pt;'>Historic ", input_params$om_stats, ":</span><br><b>", round(site_historic, 2), "%</b>")),
      linewidth = .4, linetype = "31", color = torv_blue, stat = "unique"
    ) +
    ## torv average as quiet gray dashed line
    geom_hline_interactive(
      ...
      linewidth = .4, linetype = "31", color = torv_gray_light, stat = "unique"
    ) +
    { if(draw_beeswarm)
...
\noindent{\large\textbf{\textcolor{torvgray}{Historical Results by Depth}}}

Values above the row average are shown in blue, values below in gray.

```{r, warning=FALSE, message=FALSE, ft.align="left", include=!html}
om_tbl_data <-
  om_avgs |>
  filter(measurement_name == "OM 440 As Rcvd (%)") |>
  arrange(depth, plot_date) |>
  mutate(
    date_col = if(use_month) format(as.Date(plot_date), "%b '%y") else as.character(lubridate::year(plot_date))
  )

## cap at 10 most recent periods so table never overflows the page
max_periods <- 10
recent_dates <- tail(sort(unique(om_tbl_data$plot_date)), max_periods)
om_tbl_data <- filter(om_tbl_data, plot_date %in% recent_dates)

date_cols_om <- unique(om_tbl_data$date_col)

stat_label <- stringr::str_to_title(input_params$om_stats)

om_wide <-
  om_tbl_data |>
  select(depth, date_col, avg_measurement_result) |>
  pivot_wider(names_from = date_col, values_from = avg_measurement_result) |>
  arrange(depth) |>
  rowwise() |>
  mutate(row_stat = calculate_om_stats(c_across(all_of(date_cols_om)))) |>
  ungroup()

## font color: above row mean = dark teal, below = light gray (date columns only)
## apply(MARGIN=1) returns a vector (not matrix) when there is only one date column;
## normalize to nrow(om_wide) × length(date_cols_om) in both cases
color_matrix_raw <-
  om_wide |>
  select(all_of(date_cols_om)) |>
  apply(1, function(row) {
    avg <- mean(row, na.rm = TRUE)
    ifelse(row >= avg, as.character(clr_darken(prismatic::color(torv_blue), .2)), "gray78")
  })

color_matrix <-
  if (!is.matrix(color_matrix_raw)) {
    matrix(color_matrix_raw, ncol = length(date_cols_om))
  } else {
    t(color_matrix_raw)
  }
...
om_ft_raw <- flextable(om_wide)

om_ft <-
  om_ft_raw |>
  set_header_labels(
    values = c(
      list(depth = "Depth", row_stat = paste0(stat_label, "\n(%)")),
      setNames(as.list(paste0(date_cols_om, "\n(%)")), date_cols_om)
    )
  ) |>
  width(width = w_om) |>
  align(j = 2:ncol_keys(om_ft_raw), align = "right", part = "all")

om_ft <-
  om_ft |>
  border_remove() |>
  hline_top(border = fp_border(color = torv_gray, width = 1.2), part = "header") |>
  hline_bottom(border = fp_border(color = torv_gray, width = 0.8), part = "header") |>
  hline_bottom(border = fp_border(color = torv_gray, width = 1.2), part = "body") |>
  hline(i = 1:2, border = fp_border(color = torv_gray_light, width = 0.5), part = "body") |>
  ## subtle left border to separate summary column
  border(j = ncol_keys(om_ft_raw), border.left = fp_border(color = torv_gray_light, width = 0.5), part = "all") |>
  colformat_double(digits = 2, na_str = "—") |>
  font(fontname = typeface, part = "all") |>
  bold(bold = TRUE, part = "header") |>
  bold(j = 1, bold = TRUE, part = "body") |>
  ## summary column: bold + dark teal to match historic line in charts
  bold(j = ncol_keys(om_ft_raw), bold = TRUE, part = "body") |>
  color(j = ncol_keys(om_ft_raw), color = as.character(clr_darken(prismatic::color(torv_blue), .2)), part = "body") |>
  align(j = 1, align = "left", part = "all") |>
  valign(valign = "top", part = "body") |>
  line_spacing(i = 1, space = 1.3, part = "header") |>
  hrule(rule = "atleast")

## apply font colors row by row (date columns only)
for(i_row in seq_len(nrow(om_wide))) {
  om_ft <- color(om_ft, i = i_row, j = date_cols_om, color = color_matrix[i_row, ])
}
...
om_ft |>
  ...
  width(j = 1, width = .65) |>
  hrule(rule = "atleast") |>
  height_all(height = .27)
```

\vspace{4mm}

```{r, eval=FALSE, include=FALSE}
## HEATMAP — commented out; preserved for potential future use
