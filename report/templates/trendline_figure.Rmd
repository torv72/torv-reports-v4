...
w <- (length(measurement_names) %/% 3) * 1.7 + .5
...
```{r, fig.width=6.5, fig.height=w, message=FALSE, warning=FALSE, results='asis', out.width="100%"}
...
plot_test_trend <- function(plot_measurement, is_first, draw_beeswarm = beeswarm) {
  ...
  site_avgs <- trendline_full_database %>%
    ...
    mutate(
      ...
      unit = if_else(is.na(unit), "", paste0(" ", unit)),
  ...
  trend_plot <- 
    ...
    theme(
      plot.title = element_text(size = 8.5, family =typeface),
...
# n_plots_requiring_axis <- ifelse(length(measurements_to_plot) %% 3 == 0,
#                                 3,
...
# 
# 
combined_trends <- wrap_plots(greens_ph_trends, ncol = 3, guides = "collect", heights = 2)
...
girafe(
  ...
  options = list(
    opts_tooltip = opts_tooltip(css = tooltip_css, opacity = 1, use_fill = FALSE, offx = 10, offy = 10),
...
# knitr::include_graphics(here(root_figure_location, "trendlines", glue::glue(janitor::make_clean_names("trend_plot_{{sample_type}}_{{sample_description}}"), ".png")))
```

If you need help to interpret these charts, consider the ["How to Read Trend Line Plots"](#how-to-read-trend-line-plots) section.

<details><summary>How to Read this Chart</summary>

```{r, fig.width=2.5, fig.height=2, out.width="50%", dpi=300}
make_trendline_legend(main_color = "grey30")
```
</details>
<br>
