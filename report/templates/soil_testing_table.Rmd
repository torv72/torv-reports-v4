...
filter(fertilizer_table_all, sample_description_number_1 == "{{sample_description_number_1}}") %>%
  ...
  flextable::fontsize(size = size_h, part = "header") %>%
  flextable::fontsize(size = size_b, part = "body") %>%
  flextable::fontsize(size = size_f, part = "footer") %>%
  # Specifying font again after adding footnote
  flextable::font(fontname = typeface_condensed, part = "all") %>%
  flextable::font(fontname = typeface, part = "footer") %>%
...
```{r, results='asis', include=html}
cat("<details><summary style='font-size:22pt;font-weight:600;'>Mean measurements and recommendations</summary>")
```

```{r, results='asis', include=!html}
cat("### Mean measurements and recommendations")
```
