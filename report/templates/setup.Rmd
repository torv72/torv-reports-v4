...
# Plot Theme
theme_set(theme_minimal(base_family = typeface))
theme_update(
  axis.text = element_text(family = typeface, color = torv_gray),
  panel.grid.major = element_line(linewidth = .4, color = "grey94"),
...
if (html) {
  # Tooltip Theme
  tooltip_css <- paste0("background-color:white;padding:12px;border-radius:5px;border-width:2px;color:white;border-style:solid;border-color:white;line-height:1.15;font-family:", typeface_tabular, ";font-size:1.4em;")
  
  set_girafe_defaults(
    opts_tooltip = opts_tooltip(css = tooltip_css, opacity = 1, use_fill = TRUE, offx = 10, offy = 10),
...
make_trendline_legend <- function(main_color, stat = "average", draw_beeswarm = input_params$beeswarm) {
  ...
  ggplot(legend_plot_data,
    ...
    annotate(
      ...
      color = prismatic::clr_lighten(torv_gray, .6)
    ) +
    
    ## average measurements
    geom_line(
      color = prismatic::clr_lighten(main_color, .3), 
    ...
    annotate(
      ...
      curvature = .2,
      arrow = arrow(length = unit(1.2, "mm"), type = "closed"),
      color = prismatic::clr_lighten(main_color, .6)
    ) +
...
ggsave(here(root_figure_location, "trendlines", "trend_plot_legend_mean.png"),
       trend_legend_mean, width = 2.5, height = 2, bg = "#ffffff", dpi = 300)
...
ggsave(here(root_figure_location, "trendlines", "trend_plot_legend_om.png"),
       trend_legend_om, width = 2.5, height = 2, bg = "#ffffff", dpi = 300)
...
ggsave(here(root_figure_location, "trendlines", "trend_plot_legend_mean_median.png"),
       trend_legend_mean_median, width = 2.5, height = 2, bg = "#ffffff", dpi = 300)
```
...
header_file_create <<- function(sampletype, title, trends = FALSE) {
  ...
  if(trends == TRUE) {
    ggsave(here(root_figure_location, "headers", glue("{clean_sampletype}_testing_{clean_title}_trends_plot.png")),
           header_plot, width = 5.6, height = 0.7, bg = "#ffffff", dpi = 300)
  } else {
