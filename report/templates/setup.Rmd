...
# Colors
torv_green <<- "#0aa67b" #44ad4c
torv_orange <<- "#a67b0a" #ec8c35
torv_blue <<- "#0a83a6" #0d98d6"
torv_green2 <<- "#5b921e"
torv_gray <<- "#808080"
...
make_trendline_legend <<- function(main_color, average_label = "Historic average", draw_beeswarm = beeswarm) {
  ...
  legend_plot_points <- tibble(year = rep(stable_year_range, each = 5),
  ...
  legend_plot_data %>%
    ...
    geomtextpath::geom_textpath(
      ...
      linewidth = .8, color = "#8F51C0", alpha = .8, size = 7/.pt, family = typeface
    ) + 
    { if(draw_beeswarm)
      ## raw measurements as beeswarms to avoid overlaps (varied around x)
      ggbeeswarm::geom_beeswarm(
        data = legend_plot_points,
        aes(x = rep(lubridate::as_date(lubridate::date_decimal(stable_year_range)), each = 5), y = measurement_result),
    ...
    geom_line(
      color = prismatic::clr_lighten(main_color, .3), linewidth = .6, linetype = "solid", alpha = .65 
    ) +
    geom_point(
    ...
    geomtextpath::geom_textpath(
      aes(y = torv_avg, label = average_label), 
      linetype = "21", linewidth = .5, 
      family = typeface, color = torv_gray, size = 7/.pt
    ) +
    annotate(
      geom = "text", 
...
trend_legend <- make_trendline_legend(main_color = "grey30", draw_beeswarm = beeswarm)
