...
is_installed <- sum(grepl("Lato", systemfonts::system_fonts()$family)) != 0
 
if(is_installed) {
  font_main <- "Lato"
} else {
  message("Using system default font as Lato is not installed on your system. Please install the typeface on your system and then restart your R session: https://fonts.google.com/specimen/Lato")
  font_main <- "Lato"
} 

theme_set(theme_minimal(base_family = font_main))
theme_update(
...
make_trendline_legend <<- function(main_color, average_label = "TORV Average", draw_beeswarm = beeswarm) {
  ...
  legend_plot_data %>%
    ...
    geomtextpath::geom_textpath(
     ...
     linewidth = .8, color = "#8F51C0", alpha = .75, size = 7/.pt
    ) + 
    { if(draw_beeswarm)
    ...
    geomtextpath::geom_textpath(
      ...
      family = font_main, color = torv_gray, size = 7/.pt
    ) +
    ggtext::geom_textbox(
      aes(x = lubridate::as_date("2021-11-01"), y = 2.95,
          label = "All samples"),
      hjust = 1, halign = 1, box.color = NA, fill = NA,
      alpha = .5, family = font_main, color = torv_gray, size = 2.5
    ) +
    annotate(
      geom = "curve",
    ...
    ggtext::geom_textbox(
      aes(x = lubridate::as_date("2020-01-01"), y = 2.62,
          label = "Average measurement<br>per date"),
      hjust = 0, halign = 0, box.color = NA, fill = NA,
      family = font_main, color = main_color, alpha = 1, size = 2.5
    ) +
    annotate(
      geom = "curve",
      x = as_date(lubridate::as_date("2020-08-15")), y = 2.57,
    ...
    theme(
      plot.title = element_text(size = 10, color = "grey50"),
...
header_file_create <<- function(sampletype, title, trends=FALSE) {
  ...
  header_plot <- ggplot() +
    ...
    annotate(
      ...
      label = plot_title,
      color = col, fontface = "bold", family = font_main
    ) +
    xlim(c(-0.5, 10)) +
    ylim(c(-0.5,0.5)) +
    coord_fixed() +
