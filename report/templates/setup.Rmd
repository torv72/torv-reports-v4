...
tooltip_css <- paste0("background-color:white;padding:12px;border-radius:5px;border-width:2px;color:white;border-style:solid;border-color:white;line-height:1.15;font-family:", typeface_title, ";font-size:1.4em;")
...
set_girafe_defaults(
  ...
  opts_hover_inv = opts_hover_inv(css = "opacity:.3;"),
...
make_trendline_legend <<- function(main_color, average_label = "TORV Average", draw_beeswarm = beeswarm) {
  ...
  legend_plot_data %>%
    ...
    geomtextpath::geom_textpath(
     ...
     linewidth = .8, color = "#8F51C0", alpha = .75, size = 7/.pt, family = typeface
    ) + 
    { if(draw_beeswarm)
    ...
    # ggtext::geom_textbox(
    #   aes(x = lubridate::as_date("2021-11-01"), y = 2.95,
    #       label = "All samples"),
    #   hjust = 1, halign = 1, box.color = NA, fill = NA,
    #   alpha = .5, family = typeface, color = torv_gray, size = 2.5
    # ) +
    annotate(
      geom = "text", 
    ...
    labs(x = NULL, y = NULL, title = "How to read:") +
    theme(
      plot.title = element_text(size = 10, color = "grey50", family = typeface_title),
...
header_file_create <<- function(sampletype, title, trends=FALSE) {
  ...
  header_plot <- ggplot() +
    ...
    annotate(
      ...
      color = col, fontface = "bold", family = typeface_title
    ) +
    xlim(c(-0.5, 10)) +
    ylim(c(-0.5,0.5)) +
    coord_fixed() +
