...
trend_legend_mean_median <- 
  make_trendline_legend(main_color = "grey30", stat = "mean/ median", draw_beeswarm = input_params$beeswarm) +
    ...
    geom_hline(
      aes(yintercept = 2.2),
      linetype = "21", linewidth = .5, color = torv_gray_light
    ) +
    annotate(
      geom = "label", x = lubridate::as_date("2021-01-01"), y = 2.2, label = "TORV average",
      family = typeface, color = torv_gray_light, size = font_size, label.size = 0
    )
...
header_file_create <<- function(sampletype, title, trends = FALSE) {
  ...
  ## ORIGINAL icon + color code — commented out, kept for easy revert
  ## Revert: remove the ## from the block below and comment out the new plot_title block
  # if (trends == FALSE) {
  #   rsvg::rsvg_svg(
  ...
  # }
  ## END original icon + color code

  ## NEW: two-level typographic headers — primary bold dark, secondary lighter gray, no icon
  ## section_title = primary line; location_label = secondary line (empty for headers with no location)
  if (sampletype == "OM") {
    section_title <- "Total Organic Matter"
    location_label <- str_to_title(title)
  } else if (sampletype == "Soil" & trends == TRUE) {
    section_title <- "Trends Over Time"
    location_label <- str_to_title(title)
  } else if (sampletype == "Soil") {
    ...
    section_title <- "Water Testing"
    location_label <- str_to_title(title)
  } else if (sampletype == "Turf") {
    section_title <- "Turf Growth Potential Model"
    location_label <- ""
  } else if (sampletype == "Physical") {
    section_title <- "Sand Fraction Analysis"
    location_label <- ""
  } else if (sampletype == "None") {
    section_title <- glue("{title}")
    location_label <- ""
  }
  col_primary   <- "#3a3a3a"
  col_secondary <- "#909090"
  ## END new typographic header code

  ## y positions and ylim: two-line vs single-line headers get different vertical ranges
  two_line          <- location_label != ""
  y_primary         <- if (two_line)  0.04  else  0.00
  y_secondary       <- -0.10
  ylim_vals         <- if (two_line) c(-0.16, 0.08) else c(-0.07, 0.06)
  fig_height        <- if (two_line) 0.58 else 0.44
  font_size_primary <- 5.6

  header_plot <-
    ggplot() +
    ## ORIGINAL: colored circle + icon — commented out for revert
    # geom_point(aes(x = 0, y = 0), color = col, size = 17) +
    # annotation_custom(sym.grob, xmin = -0.5, xmax = 0.5, ymin = -0.5, ymax = 0.5) +
    annotate(
      "text", x = -0.4, y = y_primary, hjust = 0,
      label = section_title, size = font_size_primary,
      color = col_primary, fontface = "bold", family = typeface
    ) +
    { if (two_line)
        annotate("text", x = -0.4, y = y_secondary, hjust = 0,
                 label = location_label, size = 4.4,
                 color = col_secondary, fontface = "plain", family = typeface)
    } +
    xlim(c(-0.5, 10)) +
    ylim(ylim_vals) +
    theme_void() +
  ...
  if(trends == TRUE) {
    ggsave(here(root_figure_location, "headers", glue("{clean_sampletype}_testing_{clean_title}_trends_plot.png")),
           header_plot, width = 5.8, height = fig_height, bg = "white", dpi = 500)
  } else {
...
if(length(soil_types > 0)) {
  ...
  source(here("aux-scripts", "soil-tiny-charts.R"))
