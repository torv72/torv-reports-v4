...
torv_green2 <<- "#5b921e"
torv_gray <<- "#808080"
torv_gray_light <<- "#B1B1B1"

# Typeface
is_installed <- sum(grepl(typeface, systemfonts::system_fonts()$family)) != 0
 
if(is_installed) {
  typeface_tabular <<- typeface#paste(typeface, "Tabular")
  ...
  message("Using system default font as the specified typeface is not installed on your system. Please install the typeface on your system and then restart your R session.")
  typeface <<- ""
  typeface_tabular <<- ""
  typeface_condensed <<- ""
}
...
theme_update(
  axis.text = element_text(family = typeface, color = torv_gray, size = base_size * .55),
  axis.text.x = element_text(size = base_size * .55, margin = margin(t = 4)),
  axis.text.y = element_text(size = base_size * .55, margin = margin(r = 4)),
  axis.title.x = element_text(size = base_size * .55),
  axis.title.y = element_text(size = base_size * .55),
  panel.grid.major = element_line(linewidth = .33, color = "grey92"),
  panel.grid.minor = element_blank(),
  plot.title = element_text(size = base_size * .75),
  strip.text = element_text(size = base_size * .75),
...
make_trendline_legend <- function(main_color, stat = "average", draw_beeswarm = input_params$beeswarm) {
  ...
  ggplot(legend_plot_data,
    ...
    geom_label(
      ...
      family = typeface, color = "#8F51C0", size = 7/.pt, label.size = 0, angle = 10, stat = "unique"
    ) +
    ...
    annotate(
      ...
      label = "All samples",
      hjust = 1, family = typeface, color = torv_gray, size = 2.5
    ) +
    annotate(
      geom = "curve",
    ...
    annotate(
      ...
      hjust = 1, family = typeface, color = main_color, size = 2.5
    ) +
    annotate(
      geom = "curve",
    ...
    geom_line(
      aes(y = torv_avg), 
      linetype = "21", linewidth = .5, color = torv_gray
    ) +
    geom_label(aes(x = lubridate::as_date("2021-01-01"), y = torv_avg, label = paste("Historic", stat)), 
      family = typeface, color = torv_gray, size = 7/.pt, label.size = 0, stat = "unique"
    ) +
    ...
    theme(
      plot.title = element_text(size = 10, color = "grey50", family = typeface),
...
trend_legend_mean <- make_trendline_legend(main_color = "grey30", draw_beeswarm = input_params$beeswarm)
...
trend_legend_om <- make_trendline_legend(main_color = "grey30", stat = input_params$om_stats, draw_beeswarm = input_params$beeswarm)
...
trend_legend_mean_median <- make_trendline_legend(main_color = "grey30", stat = "mean/ median", draw_beeswarm = input_params$beeswarm)
...
header_file_create <<- function(sampletype, title, trends = FALSE) {
  ...
  if(trends == TRUE) {
    ggsave(here(root_figure_location, "headers", glue("{clean_sampletype}_testing_{clean_title}_trends_plot.png")),
           header_plot, width = 5.6, height = 0.7, bg = "white", dpi = 500)
  } else {
...
```{r prepare-element-tables-and-auto-comments}
