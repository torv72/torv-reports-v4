...
sf_point_plot <-  
  ...
  geom_point(
    data = filter(sf_data_points_red, date_sample_submitted != max(date_sample_submitted)),
    aes(group = date_sample_submitted),
    size = .3, color = torv_gray_light
  ) +
  geom_line(
    data = filter(sf_data_points_red, date_sample_submitted != max(date_sample_submitted)),
    aes(group = date_sample_submitted),
    linewidth = .3, color = torv_gray_light
  ) +
  ...
  scale_color_manual(values = c("#B22222", torv_green), na.value = torv_gray_light, guide = "none") +
  scale_fill_manual(values = c("#B22222", torv_green), na.value = torv_gray_light, guide = "none") +
...
if (html) {
  ...
  sf_point_plot +
    geom_text(
      ...
      hjust = -.5, family = typeface_condensed, fontface = "bold", size = 2.5
    )
