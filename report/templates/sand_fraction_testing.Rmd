...
The sand fraction sampling yadda yadda yadda. The recommended ranges are taken from the USGA Recommendations for a Method of Putting Green Construction, 2018 Revision. Note that the USGA does not provide a recommendation for Sand, and thus no recommended ranges are shown in the following charts and tables.
...
sf_overview_plot <- 
  ...
  geom_text_interactive(
    ...
    hjust = 0, color = "#B22222", lineheight = .8
  ) +
...
sf_data_points_red <-
  ...
  mutate(
    ...
    point_color = case_when(
      is_satisfied == TRUE ~ torv_green,
      is_satisfied == FALSE ~ "#B22222",
...
sf_point_plot <-  
  ...
  geom_rect_interactive(
    aes(ymin = xbegin, xmin = -Inf, 
        ...
        tooltip = paste0("<span style='font-size:12pt;'>Recommended</span><br><b style=color:", clr_darken(torv_green, .3), ";>", usga_spec, "</b>"),
        data_id = measurement_name), 
    fill = torv_green, alpha = .1, stat = "unique"
  ) +
  geom_hline_interactive(
    aes(yintercept = usga_value,
        tooltip = paste0("<span style='font-size:12pt;'>Recommended</span><br><b style=color:", clr_darken(torv_green, .3), ";>", usga_spec, "</b>"),
        data_id = measurement_name), 
    color = clr_lighten(torv_green, .25)
  ) +
  ...
  scale_color_manual(values = c(`FALSE` = "#B22222", `TRUE` = torv_green), na.value = torv_gray_light, guide = "none") +
  scale_fill_manual(values = c(`FALSE` = "#B22222", `TRUE` = torv_green), na.value = torv_gray_light, guide = "none") +
...
cat('
The line in the "Trend" column shows the change of the shares by weight over time.

- <b style="color:#0AA67B;">Green dots</b> dots represent samples that meet USGA recommendations.
- <b style="color:#B22222;">Red dots</b> represent samples that do not satisfy USGA recommendations.
- <b style="color:#808080;">Gray dots</b> represent cases where no USGA recommendations are available for comparison.

The filled dots indicate the latest sample, the circles previous samples.
')
```
...
if(length(unique(sf_data_long$date_sample_submitted)) > 1) {
  plot_sparkline <- function(level, class) {
    ...
    ggplot(df, aes(x = date_sample_submitted, y = measurement_result)) +
      ...
      scale_color_manual(values = c(`FALSE` = "#B22222", `TRUE` = torv_green), na.value = torv_gray, guide = "none") +
      theme_void() +
      theme(plot.margin = margin(6, .5, 6, 2.5))
  }
  sf_data_wide <-
    sf_data_wide |> 
