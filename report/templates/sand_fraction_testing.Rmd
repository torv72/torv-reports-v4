\newpage
\vspace*{-0.8cm}
```{r, warning=FALSE, message=FALSE, out.width=if(knitr::is_latex_output()) "5.8in" else "900px", out.height=if(knitr::is_latex_output()) "0.44in" else NULL}
...
The S022 analysis measures the particle size distribution of the rootzone at three depths — 0–2 cm, 2–4 cm, and 4–6 cm — and compares each fraction against the specifications outlined in \textit{USGA Recommendations for a Method of Putting Green Construction, 2018 Revision}. Particle size governs drainage rate, surface firmness, and how quickly the profile compacts over time.

Note that the USGA specifies no target range for the Sand fraction; no recommendation line appears in those charts.
...
sf_data_wide <- 
  ...
  filter(usga_spec != "" | as.character(measurement_name) == "Sand")
```

```{r, eval=FALSE, include=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=6.8, fig.height=3.3, out.width="100%"}
## BAR CHART — commented out; replaced by improved profile plot
...
```{r, results='asis'}
if(html) cat("<br><br>") else cat("\\vspace{4mm}")
```
...
## sf_data_bars_red moved here from bar chart chunk (bar chart commented out above)
sf_data_bars_red <-
  sf_data_long |>
  filter(usga_spec != "" | measurement_name == "Sand") |>
  mutate(
  ...
  mutate(
    measurement_name = factor(measurement_name, levels = rev(levels(measurement_name))),
    usga_value = if_else(usga_value == 999, NA, usga_value),
    is_satisfied = if_else(usga_direction == 1, measurement_result > usga_value, measurement_result < usga_value),
    warning_lab = if_else(usga_direction == 1, "too low", "too high"),
    warning_lab = paste0(warning_lab, "\n", if_else((measurement_result - usga_value) > 0, "+", "-"), sprintf("%.1f", abs(measurement_result - usga_value)), " pp"),
...
sf_point_plot <-  
  ...
  facet_wrap(~ measurement_name, scales = "free_x", ncol = 3,
             labeller = labeller(measurement_name = function(x) {
               gsub("Very Fine Sand \\+ Very Fine Sand\n", "Very Fine Sand +\nVery Fine Sand\n", x)
             })) +
  coord_flip(clip = "off") +
  scale_x_reverse(
    limits = c(6, 0), expand = c(0, 0), 
    breaks = c(0, 2, 4, 6), labels = c("surface", "2 cm", "4 cm", "6 cm")
  ) +
  ...
  scale_fill_manual(values = c(`FALSE` = "#a65d57", `TRUE` = "white"), na.value = torv_gray_light, guide = "none") +
  ...
  theme(
    ...
    strip.text = element_text(vjust = 1, size = rel(0.75), lineheight = 0.9),
...
cat("<br><br><details><summary style='font-size:1.95rem;font-weight:500;'>Particle Size Trends</summary>")
```

```{r, results='asis', include=!html}
cat("\\pagebreak")
cat("\\vspace{6mm}\n\\noindent{\\large\\textbf{\\textcolor{torvgray}{Particle Size Trends}}}")
```

The table shows the particle size fractions defined by USGA specifications for putting green construction, along with the total sand fraction.
...
## date columns = everything except depth, measurement_name, usga_spec, gg
date_cols <- setdiff(names(sf_data_wide), c("depth", "measurement_name", "usga_spec", "gg"))
pct_labels <- setNames(paste0(date_cols, "\n(%)"), date_cols)

## show depth label only on first row of each group — naturally top-aligned, no merge_v needed
sf_table_raw <-
  sf_data_wide |>
  group_by(depth) |>
  mutate(depth = if_else(row_number() == 1, as.character(depth), "")) |>
  ungroup() |>
  flextable()
...
if(length(unique(sf_data_long$date_sample_submitted)) > 1) {
  ...
  sf_table <-
    ...
    mk_par(
      j = "gg",
      value = as_paragraph(
        gg_chunk(value = gg, width = f, height = .27)
      )
    ) |>
    set_header_labels(
      values = c(
        as.list(pct_labels),
        list(depth = 'Depth', measurement_name = 'Particle Size',
             usga_spec = 'USGA Spec', gg = 'Trend')
      )
    ) |>
    width(width = w) |>
...
sf_table <-
  ...
  hline(i = c(6, 12), border = fp_border(color = torv_gray_light, width = 0.5), part = "body") |>
