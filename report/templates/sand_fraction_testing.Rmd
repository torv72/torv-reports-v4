...
sf_point_plot <-  
  ...
  scale_color_manual(values = c(`FALSE` = "#B22222", `TRUE` = torv_cyan), na.value = torv_gray_light, guide = "none") +
  scale_fill_manual(values = c(`FALSE` = "#B22222", `TRUE` = torv_cyan), na.value = torv_gray_light, guide = "none") +
...
if(length(unique(sf_data_long$date_sample_submitted)) > 1) {
  plot_sparkline <- function(level, class) {
    ...
    ggplot(df, aes(x = date_sample_submitted, y = measurement_result)) +
      ...
      scale_color_manual(values = c(`FALSE` = "#B22222", `TRUE` = torv_cyan), na.value = torv_gray, guide = "none") +
      theme_void() +
